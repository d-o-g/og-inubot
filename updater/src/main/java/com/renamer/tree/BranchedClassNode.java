 ...
 *
 */
public class BranchedClassNode {
	...
	private final List<BranchedClassNode> parents;
	private final List<BranchedClassNode> children;
	...
	public BranchedClassNode(String name,
							 int access,
							 String superName,
							 boolean external,
		...
		this.parents = new ArrayList<BranchedClassNode>();
		this.children = new ArrayList<BranchedClassNode>();
	}

	public void addParent(BranchedClassNode node) {
		parents.add(node);
	}

	public void addChild(BranchedClassNode node) {
		children.add(node);
	}
	...
	public boolean renameField(MemberNode node, String name) {
		if (!fieldMap.containsKey(node)
			...
			renameFieldTree(new LinkedList<BranchedClassNode>(), node, name);
			return true;
		}
		return false;
	}
	...
	public boolean renameMethod(MemberNode node, String name) {
		if (!methodMap.containsKey(node)
			...
			renameMethodTree(new LinkedList<BranchedClassNode>(), node, name);
			return true;
		}
		return false;
	}
	...
	public List<BranchedClassNode> getParents() {
	...
	boolean isFieldRenamable(MemberNode node) {
		...
		if (myNode != null && (myNode.access &
			...
			for (BranchedClassNode parent : parents) {
	...
	void renameFieldTree(List<BranchedClassNode> visited,
	...
	void renameMethodTree(List<BranchedClassNode> visited,
						  MemberNode node, String name) {
		if (!external && !visited.contains(this)) {
