 ...
 */
package org.runedream.bot.modscript.transform.util;
...
/*    public List<AbstractInsnNode[]> deepSearch(Pattern p, InsnList stack) {
        BranchExplorer explorer = new BranchExplorer();
        List<AbstractInsnNode[]> branches = explorer.compute(stack.toArray());
        List<AbstractInsnNode[]> matches = new ArrayList<>();
        for(AbstractInsnNode[] branch : branches) {
            InsnList stack0 = new InsnList();
            for(AbstractInsnNode ain : branch)
                stack0.add(ain);
            matches.addAll(new RIS(stack0).search(p));
        }
        return matches;
    }*/
