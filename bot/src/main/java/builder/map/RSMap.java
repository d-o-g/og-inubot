...
public class RSMap extends MapScheduler {
    ...
    public final void loadMap() {
        ...
        if (RuneDream.getInstance() != null && Game.isLoggedIn()) {
            tileViewX = Players.getLocal().getX() - mapBaseX;
            tileViewY = (mapBaseY + mapHeight) - Players.getLocal().getY();
        } else {
    ...
    public final void processEvents() {
        ...
        if (RuneDream.getInstance() != null && (lastX != Players.getLocal().getX() || lastY != Players.getLocal().getY())) {
            force_repaint = true;
        }
    ...
    public final void paint() {
        if (force_repaint) {
            ...
            if (RuneDream.getInstance() != null && Game.isLoggedIn()) {
                tileViewX = Players.getLocal().getX() - mapBaseX;
                tileViewY = (mapBaseY + mapHeight) - Players.getLocal().getY();
                lastX = Players.getLocal().getX();
                lastY = Players.getLocal().getY();
            }
...
/*                           //  int maxY = destinationTopLeftY + (dest_height * (map_y - sourceTopLeftY)) / source_height;
        ...
        if (RuneDream.getInstance() != null && Game.isLoggedIn()) {
            ...
            letterPlotter.drawString("Location: " + loc.getX() + "," + loc.getY(), 20, 45, Color.GREEN.getRGB());
        }
/*
        int dx = 3211;
        int dy = 3381;
        ...
        if (drawHoveredTile) {
            ...
            if (RuneDream.getInstance() != null && Game.isLoggedIn()) {
                map_x = Players.getLocal().getX() - mapBaseX;
                map_y = (mapBaseY + mapHeight) - Players.getLocal().getY();
                minX = destinationTopLeftX + (dest_with * (map_x - sourceTopLeftX)) / source_with;
                minY = destinationTopLeftY + (dest_height * (map_y - 1 - sourceTopLeftY)) / source_height;
                maxX = destinationTopLeftX + (dest_with * (map_x + 1 - sourceTopLeftX)) / source_with;
                maxY = destinationTopLeftY + (dest_height * (map_y - sourceTopLeftY)) / source_height;
                letterPlotter.drawString(Players.getLocal().getName(), minX, maxY, Color.GREEN.getRGB());
            }
        }
    ...
    public final MapDataStore getMapData() {
        ...
        try {
            ...
            byte is[] = builder.map.cache.io.FileReader.readFile(string + DataArchive.getValue(ArchiveKeys.MAP_DATA_FILE_PATH));
    ...
    public final byte[] openStream() {
        ...
        try {
            ...
            File map = new File("./worldmap.dat");
